...
from app.schemas.dashboard import CallJobOut, CallOut, DashboardSummary, ScheduleCallRequest
...
def get_call_jobs(db: Session = Depends(get_db)) -> list[CallJobOut]:
    return list_jobs(db)


@router.post("/call-jobs", response_model=CallJobOut)
def create_call_job(payload: ScheduleCallRequest, db: Session = Depends(get_db)) -> CallJobOut:
    schedule_call(db, payload.customer_id, payload.obligation_id, payload.scheduled_at)
    return list_jobs(db)[0]


@router.get("/calls", response_model=list[CallOut])
def get_calls(db: Session = Depends(get_db)) -> list[CallOut]:
